...
func TestMain(m *testing.M) {
	db, err := sql.Open("postgres", "postgresql://root@127.0.0.1:26257/oscardb?sslmode=disable")
	if err != nil {
		log.Fatal(err)
	}
	err = db.Ping()
...
func db(t *testing.T) relstor.Provider {
	var err error
	cdbOnce.Do(func() {
		cdb, err = New("postgresql://root@127.0.0.1:26257/oscardb?sslmode=disable", "oscardb")
		if err != nil {
			t.Fatal(err)
		}
	})
